...
namespace Tetris
{
    class Z2: Shape
    {
		...
		public Z2(int cx, int cy)
		{
            ...
            this.color = DrawColor.Shade.COLOR_CYAN;
            this.originColor = DrawColor.Shade.COLOR_DK_CYAN;
            Box origin = new Box(coor_x, coor_y, color);
			Box a = new Box(coor_x - 1, coor_y - 1, color);
        ...
        public override bool setUpNewOrientation(Orientation orient)
        {
            ...
            switch (orient)
			{
				case Shape.Orientation.ORIENT_0:
                    if (coor_x + 1 > Constants.GAME_MAX_X || coor_x - 1 < Constants.GAME_MIN_X || coor_y - 1 < Constants.GAME_MIN_Y)
                    {
                        isValid = false;
                        break;
                    }
                    a = new Box(coor_x - 1, coor_y - 1, color);
                ...
                case Shape.Orientation.ORIENT_1:
                    if (coor_x - 1 < Constants.GAME_MIN_X || coor_y - 1 < Constants.GAME_MIN_Y || coor_y + 1 > Constants.GAME_MAX_Y)
                    {
                        isValid = false;
                        break;
                    }
                ...
                case Shape.Orientation.ORIENT_2:
                    ...
                    origin = new Box(coor_x, coor_y, color);
                    b = new Box(coor_x - 1, coor_y, color);
                    c = new Box(coor_x + 1, coor_y + 1, color);
